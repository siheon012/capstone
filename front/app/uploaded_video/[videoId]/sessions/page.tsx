...
import { Video, Calendar, Clock, HardDrive, MessageSquare, Trash2, Download, ArrowLeft, Plus, Play, TimerIcon as Timeline, X } from 'lucide-react'
    ...
    <div className="min-h-screen bg-[#1a1f2c] text-gray-100 flex flex-col">
      ...
      >
        ...
        <div className="mb-4 sm:mb-6">
          <Link href="/uploaded_video">
            <Button variant="ghost" className="text-gray-300 hover:text-[#00e6b4] hover:bg-[#1a1f2c]">
              <ArrowLeft className="h-4 w-4 mr-2" />
              비디오 목록으로 돌아가기
            </Button>
          </Link>
            ...
            <div className="flex flex-col sm:flex-row sm:items-start gap-4 sm:gap-6">
              ...
              <div className="flex-1 min-w-0 order-2 sm:order-2">
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-4">
                  <div>
                    <h1 className="text-xl sm:text-2xl font-bold text-white mb-2">{video.originalName}</h1>
                    {video.description && <p className="text-sm sm:text-base text-gray-400 mb-3">{video.description}</p>}
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  ...
                  <div className="flex items-center gap-2 text-xs sm:text-sm text-gray-400">
                    ...
                    <span className="truncate">{formatFileSize(video.size)}</span>
                  </div>
                ...
                <div className="flex flex-wrap gap-2">
                  <Link href={`/uploaded_video/${video.id}`}>
                    <Button className="bg-[#00e6b4] hover:bg-[#00c49c] text-[#1a1f2c] text-xs sm:text-sm h-9">
                      <Play className="h-3 w-3 sm:h-4 sm:w-4 mr-1 sm:mr-2" />새 분석 시작
                    </Button>
                  </Link>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6 gap-3 sm:gap-0">
          <div>
            <h2 className="text-xl sm:text-2xl font-bold text-white mb-1 sm:mb-2">분석 세션 목록</h2>
            <p className="text-sm text-gray-400">이 비디오로 진행된 모든 분석 세션을 확인할 수 있습니다.</p>
          </div>
          ...
          {sessions.length === 0 ? (
            <Card className="bg-[#242a38] border-0 shadow-lg">
              <CardContent className="p-6 sm:p-12 text-center">
                ...
                <h3 className="text-lg sm:text-xl font-semibold text-white mb-2">아직 분석 세션이 없습니다</h3>
                <p className="text-sm sm:text-base text-gray-400 mb-6">이 비디오로 첫 번째 분석을 시작해보세요.</p>
                <Link href={`/uploaded_video/${video.id}`}>
                  <Button className="bg-[#00e6b4] hover:bg-[#00c49c] text-[#1a1f2c]">
            ...
            sessions.map((session) => {
              ...
              const firstAssistantMessage = session.messages.find((msg) => msg.role === "assistant")
                    ...
                    <CardContent className="p-4 sm:p-6">
                      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-3 sm:mb-4 gap-2 sm:gap-0">
                        <div className="flex-1 min-w-0">
                          <h3 className="text-base sm:text-lg font-semibold text-white mb-2">{session.title}</h3>

                          {/* 첫 번째 질문과 답변 */}
                          <div className="space-y-2 mb-3 sm:mb-4">
                            {firstUserMessage && (
                              <div className="text-xs sm:text-sm">
                                <span className="text-[#6c5ce7] font-medium">Q:</span>
                                <span className="text-gray-300 ml-2">{firstUserMessage.content}</span>
                              </div>
                            )}
                            {firstAssistantMessage && (
                              <div className="text-xs sm:text-sm">
                                <span className="text-[#00e6b4] font-medium">A:</span>
                          ...
                          <div className="grid grid-cols-2 gap-2 sm:gap-4">
                            ...
                            <div className="flex items-center gap-1 sm:gap-2 text-xs text-gray-400">
                              <MessageSquare className="h-3 w-3 sm:h-4 sm:w-4 flex-shrink-0" />
                              <span className="truncate">{session.messages.length}개 메시지</span>
                            </div>
                            ...
                            {timestamps.length > 0 && (
                              <div className="flex items-center gap-1 sm:gap-2 text-xs text-gray-400 col-span-2 sm:col-span-1">
                                ...
                                <span className="truncate">
                                  {timestamps.length}개 타임라인 ({formatTime(timestamps[0])} ~{" "}
          ...
          <div className="flex-1 h-[calc(100vh-80px)] overflow-hidden">
            <DynamicHistorySidebar
              onSelectHistory={(session) => {
                if (session.videoId) {
                  window.location.href = `/uploaded_video/${session.videoId}`
                }
      ...
      >
        <div className="container mx-auto px-4 py-6 sm:py-8">
          <div className="text-center mb-4 sm:mb-6">
            <h2 className="text-xl sm:text-2xl font-bold text-[#00e6b4] mb-2 sm:mb-3">AI 기반 CCTV 영상 분석 플랫폼</h2>
            <p className="text-sm sm:text-lg text-gray-400">실시간 이벤트 감지 • 스마트 보안 솔루션 • Deep Sentinel</p>
          </div>
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-3 sm:gap-4">
            ...
            <div className="flex items-center gap-2 text-xs sm:text-base text-gray-300">
              ...
              <Button
                ...
                onClick={() => window.open("mailto:contact@deepsentinel.com", "_blank")}
              >
                Contact
              </Button>
            </div>
