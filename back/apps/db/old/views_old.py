...
class VideoViewSet(viewsets.ModelViewSet):
    ...
    def upload_video(self, request):
        ...
        try:
            ...
            logger.info(
                f"Metadata: duration={metadata['duration']}s, fps={metadata['fps']}"
            )
            ...
            if s3_key:
                ...
                video.s3_bucket = getattr(
                    settings, "AWS_STORAGE_BUCKET_NAME", "capstone-video-bucket"
                )
        ...
        try:
            ...
            if analysis_status == "completed" and progress == 100:
                try:
                    ...
                    if events.exists():
                        logger.info(
                            f"🤖 [Auto-Summary] 자동 요약 생성 시작: video_id={video.video_id}, events={events.count()}개"
                        )

                        vlm_service = get_vlm_service()
                        summary = vlm_service.generate_video_summary(
                            video=video, events=list(events), summary_type="events"
                        )
                        ...
                        logger.info(
                            f"✅ [Auto-Summary] 자동 요약 생성 완료: video_id={video.video_id}"
                        )
                    else:
                        logger.warning(
                            f"⚠️ [Auto-Summary] 이벤트가 없어 요약 생성 생략: video_id={video.video_id}"
                        )
...
class PromptSessionViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        sessions_without_id = PromptSession.objects.filter(
            session_id=""
        ) | PromptSession.objects.filter(session_id__isnull=True)
        if sessions_without_id.exists():
            count = sessions_without_id.count()
            logger.warning(
                f"⚠️ [AUTO-FIX] {count}개의 세션에 session_id가 없습니다. 자동으로 UUID를 할당합니다..."
            )
            ...
            logger.info(
                f"✅ [AUTO-FIX] {count}개의 세션 ID를 성공적으로 업데이트했습니다!"
            )
        ...
        if video_id:
            try:
                        ...
                        for interaction in interactions:
                            ...
                            for event in related_events:
                                if event.video == video and not session.related_videos:
                                    ...
                                    fixed_count += 1
                                    logger.info(
                                        f"✅ [AUTO-FIX] 세션 {session.session_id[:8]}에 비디오 {video.name} 연결"
                                    )
                                    break
                ...
                if fixed_count > 0:
                    logger.info(
                        f"✅ [AUTO-FIX] {fixed_count}개의 세션에 related_videos를 연결했습니다!"
                    )
            except Video.DoesNotExist:
                pass
            ...
            if not video:
                ...
                if first_interaction:
                    ...
                    if first_event and first_event.video:
                        ...
                        logger.info(
                            f"ℹ️ [DELETE] related_videos 없음, 인터랙션 이벤트에서 비디오 찾음"
                        )

            if video:
                video_name = video.name or video.filename or f"Video-{video.video_id}"
                logger.info(
                    f"🔥 [DELETE] 세션 삭제 요청: session_id={session_id}, video={video_name} (ID: {video.video_id})"
                )
            else:
                logger.info(
                    f"🔥 [DELETE] 세션 삭제 요청: session_id={session_id}, video=연결된 비디오 없음"
                )
                logger.warning(
                    f"⚠️ [DELETE] 경고: 세션에 비디오 정보를 찾을 수 없습니다."
                )

            logger.info(
                f"📊 [DELETE] 세션 상호작용 수: {instance.interactions.count()}"
            )
