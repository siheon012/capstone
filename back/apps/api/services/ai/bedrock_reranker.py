...
class BedrockReranker:
    ...
    def rerank(
        self, query: str, documents: List[Dict], top_k: int = 5, max_chunks: int = 30
        ...
        try:
            ...
            logger.info(
                f"✅ Rerank 완료: {len(documents)}개 → {len(reranked_results)}개 (top_k={top_k})"
            )
...
"""
            ...
            response = self.bedrock.invoke_model(
                modelId=settings.AWS_BEDROCK_MODEL_ID, body=json.dumps(body)
            )
