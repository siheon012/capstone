...
class VideoAnalysis(models.Model):
    ...
    # 분석 결과 (JSON 구조)
    detected_people = models.JSONField(default=list, help_text="[{'gender': 'male', 'age': 25, 'confidence': 0.9}]")
    detected_actions = models.JSONField(default=list, help_text="[{'action': 'walking', 'confidence': 0.8}]")
    detected_objects = models.JSONField(default=list, help_text="[{'object': 'bag', 'confidence': 0.7}]")
    scene_description = models.TextField(help_text="Human-readable scene description for RAG")
    
    # Bedrock 임베딩 (pgvector 설치 전까지 JSON으로 저장)
    embedding_json = models.JSONField(null=True, blank=True, help_text="Bedrock embedding vector as JSON")
    embedding_model = models.CharField(max_length=100, default='amazon.titan-embed-text-v1')
    
    # Hot/Warm/Cold 계층 관리
    storage_tier = models.CharField(max_length=10, default='hot', choices=[
    ...
    confidence_score = models.FloatField(default=0.0, help_text="Overall confidence of this analysis")
