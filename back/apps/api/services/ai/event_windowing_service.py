...
class EventWindowingService:
    ...
    def create_windowed_text(self, event: Event) -> str:
        ...
        try:
            # 같은 비디오의 앞뒤 이벤트 가져오기
            video_events = Event.objects.filter(video=event.video).order_by("timestamp")
            ...
            logger.debug(
                f"✅ 윈도잉 완료: Event {event.id}, 윈도우 크기: {len(window_events)}"
            )
    ...
    def create_event_sequence_summary(
        self, events: List[Event], max_events: int = 10
    ) -> str:
        """
        연속된 이벤트들을 시간 순으로 요약
    ...
    def group_by_temporal_proximity(
        self, events: List[Event], time_gap_threshold: float = 5.0
    ) -> List[List[Event]]:
        """
        시간적으로 가까운 이벤트들을 그룹화
    ...
    def create_summarized_events(
        self, events: List[Event], bedrock_service=None
    ) -> List[Dict]:
        """
        ...
        logger.info(
            f"✅ 이벤트 요약 완료: {len(events)}개 → {len(summarized_events)}개 요약"
        )
