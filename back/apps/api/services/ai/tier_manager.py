...
class TierManager:
    ...
    def move_to_warm(self, analysis):
        ...
        try:
            ...
            warm_data = {
                ...
                "last_accessed": (
                    analysis.last_accessed.isoformat()
                    if analysis.last_accessed
                    else None
                ),
            ...
            if analysis.storage_tier == "warm":
                ...
                if hasattr(analysis, "s3_warm_key"):
                    response = self.s3_client.get_object(
                        Bucket=settings.S3_WARM_BUCKET, Key=analysis.s3_warm_key
                    )
                    warm_data = json.loads(response["Body"].read())

                    # 임베딩 재생성 (필요한 경우)
                    if not analysis.embedding_json:
                        embedding = self.regenerate_embedding(
                ...
                logger.warning(
                    f"Cold storage promotion for {analysis.id} requires Glacier restore"
                )
    ...
    def run_daily_tier_management(self):
        ...
        logger.info(
            f"Tier management completed: {promoted} promoted, {demoted} demoted"
        )
