...
class SQSVideoProcessingService:
    ...
    def __init__(self):
        ...
        if self.use_localstack:
            ...
            logger.info(
                f"LocalStack SQS 클라이언트 초기화 완료 - endpoint: {endpoint_url}"
            )
        else:
    ...
    def send_video_processing_message(
        ...
        try:
            ...
            message_body = {
                ...
                "s3": {"bucket": s3_bucket, "key": s3_key},
                "video": {"id": video_id, "status": "pending"},
                "processing": {
            ...
            response = self.sqs_client.send_message(
                ...
                MessageAttributes={
                    "video_id": {"StringValue": str(video_id), "DataType": "String"},
                    "event_type": {
            ...
            logger.info(
                f"SQS 메시지 발송 성공: MessageId={response['MessageId']}, video_id={video_id}"
            )
