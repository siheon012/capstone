...
def trigger_video_analysis(video_id, s3_key, s3_bucket):
    ...
    if not video_analysis_url:
        print(f"⚠️ [Video Analysis] VIDEO_ANALYSIS_URL이 설정되지 않았습니다. 분석을 건너뜁니다.")
        return False
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✅ [Video Analysis] 분석 요청 성공: video_id={video_id}, job_id={result.get('job_id')}")
            return True
        else:
            print(f"❌ [Video Analysis] 분석 요청 실패: status={response.status_code}, response={response.text}")
            return False
