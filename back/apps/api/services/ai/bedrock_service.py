    ...
    for field in Event._meta.get_fields():
        ...
        if field_name in detailed_info:
            field_descriptions.append(
                f"        - {field_name}: {detailed_info[field_name]}"
            )
        elif field_name in ["id", "video_id", "video"]:
            # Primary/Foreign key는 간단하게
            if field_name == "id":
                field_descriptions.append(
                    f"        - {field_name}: {sql_type} (Primary Key)"
                )
            elif field_name == "video_id" or field_name == "video":
                field_descriptions.append(
                    f"        - video_id: INTEGER (Foreign Key -> db_video.video_id)"
                )
        else:
            # 기타 필드
...
class BedrockService:
    ...
    def __init__(self):
        ...
        logger.info(
            f"✅ Bedrock 서비스 초기화: region={self.region}, model={self.model_id}"
        )
    ...
    def text_to_sql(
        self, prompt: str, video_id: Optional[int] = None
    ) -> Dict[str, any]:
        """
...
JSON 형식으로만 응답하세요."""
    ...
    def format_timeline_response(
        self, prompt: str, events: List[Dict], video_name: str = None
    ) -> str:
        """
...
답변:"""
    ...
    def retrieve_from_knowledge_base(
        self, query: str, knowledge_base_id: str = None, max_results: int = 5
    ) -> List[Dict]:
        """
    ...
    def generate_embedding(self, text: str) -> Optional[List[float]]:
        ...
        try:
            ...
            if embedding and len(embedding) == 1024:
                ...
                logger.warning(
                    f"⚠️ 예상치 못한 임베딩 차원: {len(embedding) if embedding else 0}"
                )
                return None
