...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--force',
            action='store_true',
            help='이미 embedding이 있는 이벤트도 재생성'
        )
        ...
        for idx, event in enumerate(queryset, 1):
            try:
                ...
                if embedding:
                    ...
                    if idx % 10 == 0:
                        self.stdout.write(self.style.SUCCESS(f"  ✅ Event {event.id} 완료"))
                else:
                    failed += 1
                ...
                failed += 1
                self.stdout.write(self.style.ERROR(f"  ❌ Event {event.id} 오류: {str(e)}"))
