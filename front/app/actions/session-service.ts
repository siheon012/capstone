...
export async function getAllSessions(): Promise<SessionResponse> {
  try {
    ...
    const dummyData: ChatSession[] = [
      ...
      },
      {
        ...
        messages: [
          ...
          { role: "assistant", content: "현재 주차장에는 총 12대의 차량이 있습니다.", timestamp: 1200 },
        ],
        videoInfo: {
          name: "주차장_CCTV_2024년1월1일.mp4",
          duration: 3600,
    ...
    return { success: false, data: [], error: "세션을 불러오는 중 오류가 발생했습니다." }
  }
}

// 특정 비디오의 세션 가져오기
export async function getVideoSessions(videoId: string): Promise<SessionResponse> {
  try {
    ...
    const videoSessions = allSessions.data.filter((session) => session.videoId === videoId)
    return { success: true, data: videoSessions }
  } catch (error) {
    console.error("Video sessions fetch error:", error)
    return { success: false, data: [], error: "비디오 세션을 불러오는 중 오류가 발생했습니다." }
  }
}
...
// 새 세션 저장
export async function saveSession(session: Omit<ChatSession, "id" | "createdAt">): Promise<string | null> {
  try {
    // 실제로는 데이터베이스에 저장
    const sessionId = `session_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`
    console.log("세션 저장:", { id: sessionId, ...session, createdAt: new Date() })
    return sessionId
  } catch (error) {
