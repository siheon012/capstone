...
class Event(models.Model):
    ...
    # 연관 비디오
    video = models.ForeignKey('db.Video', on_delete=models.CASCADE, related_name='events')
    ...
    # 인구통계 정보
    age_group = models.CharField(max_length=20, blank=True, help_text="young, middle, old")
    gender = models.CharField(max_length=10, blank=True, help_text="male, female")
    emotion = models.CharField(max_length=20, blank=True, help_text="happy, neutral, sad")
    
    # 행동 분석
    action = models.CharField(max_length=100, blank=True, help_text="walking, standing, picking")
    interaction_target = models.CharField(max_length=100, blank=True, help_text="Interaction target")
    confidence = models.FloatField(default=0.0, help_text="Confidence score 0-1")
    
    # 추가 메타데이터
    attributes = models.JSONField(default=dict, blank=True, help_text="Additional attributes")
