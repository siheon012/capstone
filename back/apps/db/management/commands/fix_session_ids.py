...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.WARNING("🔍 session_id가 없는 세션들을 찾는 중...")
        )
        ...
        if count == 0:
            self.stdout.write(
                self.style.SUCCESS("✅ 모든 세션이 session_id를 가지고 있습니다.")
            )
            return

        self.stdout.write(
            self.style.WARNING(f"⚠️  {count}개의 세션에 session_id가 없습니다.")
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"✅ {updated_count}개의 세션 ID를 성공적으로 업데이트했습니다!"
            )
        )
        ...
        if remaining > 0:
            self.stdout.write(
                self.style.ERROR(
                    f"❌ 여전히 {remaining}개의 세션에 session_id가 없습니다."
                )
            )
        else:
            self.stdout.write(
                self.style.SUCCESS("✅ 모든 세션이 이제 session_id를 가지고 있습니다!")
            )
