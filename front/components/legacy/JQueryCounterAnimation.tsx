...
export default function JQueryCounterAnimation({
  stats,
            ...
            try {
              $({ countNum: 0 }).animate(
                { countNum: targetValue },
                {
                  ...
                  step: function () {
                    $element.text(
                      Math.floor(this.countNum).toLocaleString() + suffix
                    );
                  },
                  complete: () => {
            ...
            (entries) => {
              entries.forEach((entry) => {
                if (entry.isIntersecting && !hasBeenVisible) {
                  ...
                  $counters.each(function (this: HTMLElement, index: number) {
                    const $this = $(this);
  ...
  return (
    ...
    >
      {stats.map((stat, index) => (
        <Card
          key={index}
          className="counter-card bg-[#242a38] border-0 shadow-lg transition-transform"
        >
