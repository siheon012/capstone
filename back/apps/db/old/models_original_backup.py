...
class Video(models.Model):
    ...
    time_in_video = models.DateTimeField(
    ...
    summary = models.TextField(
        null=True, blank=True
    )  # 영상 분석 모델의 최종 분석 결과 (이모지, 특수기호 포함 가능)
    ...
    def delete(self, *args, **kwargs):
        ...
        total_interactions = sum(
            session.interactions.count() for session in self.prompt_sessions.all()
        )
        ...
        thumbnail_file_path = os.path.join(
            settings.THUMBNAIL_DIR, f"{name_without_ext}.png"
        )
            ...
            for session in self.prompt_sessions.all():
                for interaction in session.interactions.all():
                    if interaction.highlight_thumbnail_path:
                        print(
                            f"프롬프트 하이라이트 썸네일: {interaction.highlight_thumbnail_path}"
                        )
        ...
        print(
            f"└─ 총 {depth_data_count}개 공간 정보, {display_data_count}개 진열대 정보 삭제"
        )
...
class Event(models.Model):
    ...
    video = models.ForeignKey(
        Video, on_delete=models.CASCADE, related_name="events", db_column="video_id"
    )
    timestamp = (
        models.IntegerField()
    )  # 영상 시작점부터 몇 초에 이벤트 발생했는지 (초 단위)
...
class PromptSession(models.Model):
    ...
    video = models.ForeignKey(
        Video, on_delete=models.CASCADE, related_name="prompt_sessions"
    )
    ...
    def get_session_number(self):  # ← 누락된 메서드 추가
        ...
        earlier_sessions = PromptSession.objects.filter(
            video=self.video, created_at__lt=self.created_at
        ).count()
    ...
    def display_title(self):
        ...
        session_number = self.get_session_number()
        return (
            f"{self.video.name}의 {session_number}번째 채팅"  # "채팅팅" → "채팅" 수정
        )
        ...
        if interaction_count > 0:
            ...
            for interaction in self.interactions.all():
                if interaction.highlight_thumbnail_path:
                    ...
                    print(
                        f"상호작용 #{interaction.id}의 썸네일: {interaction.highlight_thumbnail_path}"
                    )

        # 데이터베이스에서 세션 삭제 (CASCADE로 상호작용들도 함께 삭제됨)
        super().delete(*args, **kwargs)
        print(
            f"세션 삭제 완료: {session_info} (총 {interaction_count}개 상호작용 함께 삭제됨)"
        )
...
class PromptInteraction(models.Model):
    ...
    highlight_thumbnail_path = models.CharField(
        max_length=500, blank=True
    )  # 타임라인 하이라이트 썸네일
    thumbnail_extracted_at = models.DateTimeField(
        null=True, blank=True
    )  # 썸네일 추출 시간
    ...
    timeline_points = models.JSONField(
        null=True, blank=True
    )  # 추출된 정확한 시간 지점들 [{"time": "10:30", "event_type": "person_detected"}, ...]
    ...
    def interaction_number(self):
        ...
        earlier_interactions = PromptInteraction.objects.filter(
            session=self.session, timestamp__lt=self.timestamp
        ).count()
...
class DepthData(models.Model):
    ...
    video = models.ForeignKey(
        Video, on_delete=models.CASCADE, related_name="spatial_data"
    )

    # 프레임 정보
    frame_name = models.CharField(
        max_length=255
    )  # 예: "20250526_182739_3fps_frame78.jpg"
    ...
    def __str__(self):
        return (
            f"SpatialData: {self.frame_name} mask#{self.mask_id} in {self.video.name}"
        )
...
class DisplayData(models.Model):
    ...
    description = models.CharField(
        max_length=255
    )  # 예: "Farthest display", "Left side display"
