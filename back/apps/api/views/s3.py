...
def request_upload_url(request):
    ...
    try:
        ...
        presigned_url, s3_key = s3_service.generate_presigned_upload_url(
            token=upload_token, content_type=content_type
        )
...
def confirm_upload(request):
    ...
    try:
        ...
        try:
            logger.info(
                f"📦 Video 데이터 준비: file_name={token_payload['file_name']}, s3_key={s3_key}"
            )
            ...
            if thumbnail_url:
                ...
                if "s3" in thumbnail_url and ".amazonaws.com/" in thumbnail_url:
                    ...
                    thumbnail_s3_key = unquote(
                        thumbnail_s3_key.split("?")[0]
                    )  # 쿼리 파라미터 제거
                else:
                    # 이미 키 형식이면 그대로 사용
            ...
            logger.info(
                f"🖼️ Thumbnail S3 key: {thumbnail_s3_key[:100]}..."
            )  # 처음 100자만 로깅
        ...
        if sqs_result["success"]:
            logger.info(
                f"SQS 메시지 발송 성공: video_id={video.video_id}, message_id={sqs_result['message_id']}"
            )
        else:
            logger.error(
                f"SQS 메시지 발송 실패: video_id={video.video_id}, error={sqs_result['error']}"
            )
...
def get_video_download_url(request, video_id):
    ...
    try:
        ...
        return Response(
            {"download_url": download_url, "expires_in": 3600},  # 1시간
            status=status.HTTP_200_OK,
        )

    except Video.DoesNotExist:
        return Response(
            {"error": "존재하지 않는 비디오입니다."}, status=status.HTTP_404_NOT_FOUND
        )
    except Exception as e:
...
def upload_thumbnail(request):
    ...
    try:
        ...
        thumbnail_url = s3_client.generate_presigned_url(
            "get_object",
            Params={"Bucket": thumbnails_bucket, "Key": s3_key},
            ExpiresIn=7 * 24 * 3600,  # 7일
        )

        logger.info(
            f"🖼️ 썸네일 업로드 완료: s3_key={s3_key}, bucket={thumbnails_bucket}"
        )

        return Response(
            {"success": True, "thumbnail_url": thumbnail_url, "s3_key": s3_key},
            status=status.HTTP_201_CREATED,
        )
