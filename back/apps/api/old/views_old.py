...
from apps.db.serializers import (
    VideoSerializer,
    EventSerializer,
...
def health_check(request):
    ...
    health_status = {
        "status": "healthy",
        "timestamp": None,
        "checks": {"database": "unknown", "pgvector": "unknown", "s3": "unknown"},
        "details": {},
    }
    ...
    try:
        ...
        if not session_id and relevant_event and not history.main_event:
            ...
            if video and relevant_event.video == video:
                ...
                logger.warning(
                    f"⚠️ 경고: 다른 비디오의 이벤트가 반환됨. 세션 비디오: {video.name if video else 'None'}, 이벤트 비디오: {relevant_event.video.name}"
                )
...
def process_vlm_chat(request):
    ...
    try:
        ...
        if not video_id:
            return Response(
                {"error": "비디오 ID가 필요합니다."}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if session_id:
            try:
                ...
                if (
                    session.related_videos
                    and session.related_videos.video_id != video.video_id
                ):
        ...
        if any(
            ...
            for keyword in ["장면", "묘사", "무슨 일", "설명", "상황"]
        ):
        ...
        # 타임라인 추출 요청
            ...
            keyword in prompt_text.lower()
            for keyword in ["타임라인", "시간", "언제", "몇 분", "몇 초"]
        ):
        ...
        # 위치별 분석 요청
            ...
            keyword in prompt_text.lower()
            for keyword in ["위치", "어디", "왼쪽", "중간", "오른쪽", "장소"]
        ):
        ...
        # 일반 질문 - 하이브리드 RAG 사용
            ...
            response_text = hybrid_search.search_and_generate(
                query=prompt_text, video_id=video_id
            )
...
def process_prompt_logic(prompt_text, video=None):
    ...
    try:
        ...
        # ============================================
            ...
            return (
                "Bedrock이 비활성화되어 있습니다. USE_BEDROCK=true로 설정하세요.",
                None,
            )
        ...
        logger.info(
            f"✅ Event 객체: {len(found_events)}개, 쿼리 결과: {len(query_results_data)}개"
        )
