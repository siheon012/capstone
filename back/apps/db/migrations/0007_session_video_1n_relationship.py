...
def check_and_add_video_field(apps, schema_editor):
    ...
    with connection.cursor() as cursor:
        ...
        print(
            f"🔍 DB 상태: PromptSession video_id={'존재' if session_has_video else '없음'}, PromptInteraction video_id={'존재' if interaction_has_video else '없음'}"
        )
...
def migrate_manytomany_to_foreignkey(apps, schema_editor):
    ...
    with connection.cursor() as cursor:
        ...
        print(
            f"🔍 Through 테이블 상태: Session={'존재' if session_through_exists else '없음'}, Interaction={'존재' if interaction_through_exists else '없음'}"
        )
    ...
    for session in PromptSession.objects.all():
        try:
            ...
            if first_video:
                ...
                migrated_sessions += 1
                print(
                    f"✅ Session {session.session_id} -> Video {first_video.video_id}"
                )
            else:
    ...
    print(
        f"📊 PromptSession: {migrated_sessions}개 마이그레이션, {orphan_sessions}개 삭제"
    )
    ...
    for interaction in PromptInteraction.objects.all():
        try:
            ...
            if first_video:
                ...
                print(
                    f"✅ Interaction {interaction.interaction_id} -> Video {first_video.video_id}"
                )
            else:
    ...
    print(
        f"📊 PromptInteraction: {migrated_interactions}개 마이그레이션, {orphan_interactions}개 삭제"
    )
