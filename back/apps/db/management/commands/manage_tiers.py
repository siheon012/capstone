...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "action", choices=["analyze", "migrate", "status"], help="Action to perform"
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
    ...
    def analyze_temperatures(self, tier_manager):
        ...
        if stats["needs_promotion"]:
            ...
            for analysis in stats["needs_promotion"][:5]:  # 처음 5개만
                self.stdout.write(
                    f"   - Analysis {analysis.id}: hotness={analysis.hotness_score:.1f}, tier={analysis.storage_tier}"
                )
    ...
    def show_status(self, tier_manager):
        ...
        self.stdout.write(f"\n🌡️  Data Distribution:")
        self.stdout.write(
            f"   🔥 Hot: {hot_count} ({hot_count/total_analyses*100:.1f}%)"
        )
        self.stdout.write(
            f"   🌤️  Warm: {warm_count} ({warm_count/total_analyses*100:.1f}%)"
        )
        self.stdout.write(
            f"   ❄️  Cold: {cold_count} ({cold_count/total_analyses*100:.1f}%)"
        )

        self.stdout.write(f"\n🔍 Search Statistics:")
        self.stdout.write(
            f"   Average searches per item: {search_stats['avg_searches']:.1f}"
        )
        self.stdout.write(
            f"   Most searched item: {search_stats['max_searches']} searches"
        )
        self.stdout.write(
            f"   Least searched item: {search_stats['min_searches']} searches"
        )
